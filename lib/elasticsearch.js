...
function getDocsToDelete (tagId, cb) {
  ...
  client.search({
    ...
    // console.log(err, response, response.hits);
    if (!err && response && response.hits && response.hits.total > 0) {
      var result = [];
 ...
 * @returns {Promise}
 */
module.exports = function insertObject (object, cb) {
...
module.exports.getDocsToDelete = getDocsToDelete; // just for testing completeness
